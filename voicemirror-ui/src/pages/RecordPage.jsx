...
export default function RecordPage() {
  ...
  const [showTherapist, setShowTherapist] = useState(false)  // 👈 НОВЫЙ STATE
  ...
  const toggleRecord = useCallback(async () => {
    if (state === 'recording') {
      ...
      try {
        ...
        setMicrophoneError('Cannot access microphone. Please check permissions.')
      }
    }
  }, [state])
  ...
  const analyze = useCallback(async () => {
    ...
    setState('analyzing')
    setApiError(null)
    const res = await uploadAudio(fileObj, note)  // 👈 передаём note в API
    if (res.error) { 
    ...
    saveEntry({ 
      ...
      advice: res.advice,  // 👈 сохраняем совет
    })
    setState('done')
  }, [fileObj, note])
  ...
  const reset = () => {
    ...
    setApiError(null)
    setMicrophoneError(null)
    setShowTherapist(false)  // 👈 закрываем чат при сбросе
  }
  ...
  return (
    <div className={s.root}>
      <div className={s.hero}>
        ...
        <p className={s.heroSub}>Speak freely for 10–60 seconds. MindVoice analyzes your pitch, energy, and pauses to detect your emotional state.</p>
      </div>
            ...
            <>
              <div className={s.recordOuter}>
                ...
                <button 
                  ...
                  disabled={microphoneError !== null}
                >
                  {isActive ? '⏹' : '🎙'}
                </button>
              </div>
              <div className={s.statusRow}>
                {state === 'idle' && !microphoneError && <span className={s.statusTxt}>TAP TO START RECORDING</span>}
                {state === 'idle' && microphoneError && <span className={s.statusError}>{microphoneError}</span>}
        ...
        <>
          ...
          {result.advice && (
            <div className={s.adviceCard}>
              ...
              {result.quote && (
                <div className={s.adviceQuote}>✨ {result.quote}</div>
              )}
              {result.grounding && (
